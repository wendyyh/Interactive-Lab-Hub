...
- Add UI screens with readable typography, question-answer progress
- Build a physical enclosure in a desktop game station style for clearer affordances (where to stand and speak to) and a cohesive look/feel.
- Reduce the LLM loop with shorter system prompt, constrained output, and local intent handling to keep turns faster.

### 2. What are other modes of interaction _beyond speech_ that you might also use to clarify how to interact?

*Visual Interaction*
- Adding progress cues: In live trials, users couldn’t tell how far along they were in the 20Q round; a mid-game recap or visible progress helped comprehension.
- Onboarding cues: Despite the proactive invite from the chat bot detecting users saying “Ready”, asking for a clearer “Start” affordance (text cues on screen) in addition to voice.

### 3. Make a new storyboard, diagram and/or script based on these reflections.
![3b-storyboard](proj_docs/3b-storyboard.png)

_Images generated by Gemini 2.5 Flash_
...
By adding a system prompt, we were able to make the interactive system fully autonomous except for the launching process. The only remaining manual step now is to power on the device and launch the Pi program by running “python bot.py” in the terminal.
...
We designed and iterated the screens in Figma.

https://www.figma.com/design/S7R1f6ORhcNgwyc6eABZxy/IDD?node-id=54-1045&t=5yXlR8OAdLvuNfxU-1

![UI](proj_docs/UI.png)

### Physical Prototyping
We used cardboards to make the physical enclosure of the device. 

First Iteration using the cardboard:

![process](proj_docs/process.jpg)

After Decoration: 

![physical](proj_docs/physical-2.jpg)

With Screen Lightup:

![prototype](proj_docs/final-prototype.jpg)
...
We asked two friends to try the 20Q chatbot in our lab setup: they approached the corner station, were invited by the presence-triggered prompt (WoZ utilized), and played full rounds answering only “yes/no” (fully autonomous by the bot). We observed timing/latency and the automatic speech recognition accuracy to test what worked well about the system and what didn’t. 

Feedback from participants:
- *Participant 1:*
 “The voice felt unnatural, and the timing between my answers and the next question was long. Sometimes the bot misheard me when I said ‘no,’ though — maybe because I didn’t speak loudly enough. It was fun overall.”
- *Participant 2:*
 “It felt interactive. But after a few questions, it repeated one question it already asked, maybe it didn’t process my earlier answer? The animations on the screen made it easy to follow the progress of the game.”
...
From these trials, we would like to expand the system with built-in repair strategies if possible: when ASR confidence is low, the bot may auto-prompt “Did you mean Yes or No?” and surface button options for quick clarification. We should also add multimodal redundancy so users aren’t blocked by noise or mishears (e.g., large on-screen “Yes/No” answer detection and button options for Start/Play Again control. To fully remove the wizarding, we need to automate launch by creating a systemd service that auto-starts bot.py. It could wire an optional hardware Start button on the physical enclosure. Finally, we could add a display of brief privacy notes on web cam’s usage (“presence only, no images stored”) to make users feel safe and intentional.
